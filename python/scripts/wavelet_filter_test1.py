...
class WaveletEqualizerApp(QMainWindow):
    ...
    def create_display_panel(self):
        ...
        self.signal_chart_view = QChartView()
        self.signal_chart_view.setRenderHint(QPainter.Antialiasing)
        self.signal_layout.addWidget(self.signal_chart_view)
        ...
        # 添加滚动区域以适应多个系数图
        self.coeff_scroll = QScrollArea()
        self.coeff_scroll_widget = QWidget()
        self.coeff_scroll_layout = QVBoxLayout()
        self.coeff_scroll_widget.setLayout(self.coeff_scroll_layout)
        self.coeff_scroll.setWidget(self.coeff_scroll_widget)
        self.coeff_scroll.setWidgetResizable(True)

        self.coeff_layout.addWidget(self.coeff_scroll)
        ...
        self.spectrum_chart_view = QChartView()
        self.spectrum_chart_view.setRenderHint(QPainter.Antialiasing)
        self.spectrum_layout.addWidget(self.spectrum_chart_view)
    ...
    def generate_sample_data(self):
        ...
        # 创建包含多个频率成分的合成信号
        signal_clean = 0.1 * np.sin(2 * np.pi * 0.01 * t)
        # 添加噪声
        noise = 0.001 * np.random.normal(size=len(t))
        pulse = 20 * (t - 25) * np.exp(- 400 * (t - 25) ** 2) + \
                20 * (t - 75) * np.exp(- 400 * (t - 75) ** 2)
        self.original_signal = signal_clean + noise + pulse
    ...
    def plot_original_signal(self):
        ...
        for i in range(len(self.original_signal)):
            # 采样以减少数据点数量，提高性能
            if i % 10 == 0:
                series.append(t[i], self.original_signal[i])
        ...
        self.signal_chart_view.setChart(chart)

    def plot_wavelet_coefficients(self):
        """绘制小波系数"""
        # 清除现有图表
        for i in reversed(range(self.coeff_scroll_layout.count())):
            widget = self.coeff_scroll_layout.itemAt(i).widget()
            if widget is not None:
                widget.deleteLater()
        ...
        for i, coeff in enumerate(self.wavelet_coeffs):
            ...
            self.coeff_scroll_layout.addWidget(chart_view)

    def plot_spectral_analysis(self):
        """绘制频谱分析"""
        chart = QChart()
        chart.setTitle("原始信号功率谱密度")
        ...
        axis_y = QValueAxis()
        axis_y.setTitleText("PSD [V**2/Hz]")
        chart.addAxis(axis_y, Qt.AlignLeft)
        series.attachAxis(axis_y)

        chart.legend().setVisible(True)
        self.spectrum_chart_view.setChart(chart)
    ...
    def plot_signal_comparison(self, original, reconstructed):
        ...
        series_original.setName("原始信号")

        # 重构信号
        series_reconstructed = QLineSeries()
        series_reconstructed.setName("重构信号")

        # 残差信号
        series_residual = QLineSeries()
        series_residual.setName("残差")
        ...
        for i in range(len(original)):
            if i % 10 == 0:
                series_original.append(t[i], original[i])
                series_reconstructed.append(t[i], reconstructed[i])
                series_residual.append(t[i], residual[i])

        # 设置不同的颜色
        pen_original = QPen(QColor(0, 0, 255))  # 蓝色
        pen_original.setWidth(1)
        series_original.setPen(pen_original)

        pen_reconstructed = QPen(QColor(255, 0, 0))  # 红色
        pen_reconstructed.setWidth(1)
        series_reconstructed.setPen(pen_reconstructed)

        pen_residual = QPen(QColor(0, 255, 0))  # 绿色
        pen_residual.setWidth(1)
        series_residual.setPen(pen_residual)

        chart.addSeries(series_original)
        chart.addSeries(series_reconstructed)
        ...
        series_reconstructed.attachAxis(axis_x)
        series_residual.attachAxis(axis_x)

        axis_y = QValueAxis()
        axis_y.setTitleText("幅值")
        chart.addAxis(axis_y, Qt.AlignLeft)
        series_original.attachAxis(axis_y)
        series_reconstructed.attachAxis(axis_y)
        series_residual.attachAxis(axis_y)
